...
export const expandCodeAt = (hunks, rawCodeOrLines, lineNumber, linesBefore, linesAfter) => {
    const linesOfCode = typeof rawCodeOrLines === 'string' ? rawCodeOrLines.split('\n') : rawCodeOrLines;
    const start = Math.max(lineNumber - linesBefore, 1);
    const slicedLines = linesOfCode.slice(
        start - 1,
        lineNumber - 1 + linesAfter
    );
