...
import {Diff, Hunk, expandCodeAt, addStubHunk, getChangeKey, markCharacterEdits} from '../src';
...
export default class File extends PureComponent {
    ...
    @bind()
    loadCollapsedCode(start, lines) {
        const {hunks} = this.state;
        const hunksWithoutStub = last(hunks).content === 'STUB' ? hunks.slice(0, -1) : hunks;
        const newHunks = expandCodeAt(hunksWithoutStub, rawCodeLines, start, 0, lines);
